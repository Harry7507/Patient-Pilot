...
class Medication(Base):
    ...
    intake_session_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID, ForeignKey("intake_sessions.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    fhir_json: Mapped[Optional[Dict[str, Any]]] = mapped_column(PG_JSON, nullable=True, default=dict)
...
class TriageResult(Base):
    ...
    intake_session_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID, ForeignKey("intake_sessions.id", ondelete="CASCADE"), unique=True, nullable=False, index=True
    )
    triage_level: Mapped[str] = mapped_column(String(50), nullable=False)  # EMERGENCY | HIGH_PRIORITY | ROUTINE
    triggered_rules: Mapped[List[Dict[str, Any]]] = mapped_column(PG_JSON, default=list, nullable=False)
